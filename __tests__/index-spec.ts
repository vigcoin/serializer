import { HASH_LENGTH } from "@vigcoin/types";
import { BufferStreamReader } from "../src/reader";
import { BufferStreamWriter } from "../src/writer";

import assert = require("assert");
import { PurgeZeroByte } from "../src/common";
...
describe("test serializer", () => {
  test("Should write/read types and Buffer", async () => {
    ...
    assert(!reader.readBoolean());
    assert(Math.floor(1000) === Math.floor(reader.readDouble()));
  ...
  test("Should create  writer with default buffer", async () => {
  ...
  test("Should read / write varint uint64", async () => {
    const writer = new BufferStreamWriter(Buffer.from([]));
    writer.writeVarintBuffer(Buffer.from([1, 2, 3, 4, 5, 6, 7, 8]));
    const varint = writer.getBuffer();

    const reader = new BufferStreamReader(varint);
    const buffer = reader.readVarintUInt64();
    assert(buffer.equals(Buffer.from([1, 2, 3, 4, 5, 6, 7, 8])));
  });
